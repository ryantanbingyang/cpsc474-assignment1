...
public class CribbageHand implements Iterable<CribbageCard>
{
    ...
    /**
     * Creates a new CribbageHand containing the union of the given
     ...
     *
     * @param h1 a CribbageHand, non-null
     * @param h2 a CribbageHand, non-null
     */
	...
	Map<CribbageCard, Integer> cardCount = new HashMap<>();
	...
	for (CribbageHand part : split)
		...
		for (CribbageCard c : part)
		    {
			if (!cardCount.containsKey(c) || cardCount.get(c) == 0)
			    {
				return false;
			    }
			cardCount.put(c, cardCount.get(c) - 1);
